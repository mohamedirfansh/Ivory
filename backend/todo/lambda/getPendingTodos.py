...
class RequestResponseProcessor:
    ...
    def validateRequest(self):
        ...
        if not set(self._requiredAttributes).issubset(set(self._unvalidatedRequest.keys())):
            log("[GetPendingTodos VALIDATION] Failed due to required Attributes not present #1, required: " + \
            ...
            if len(set(self._unvalidatedRequest.keys()).intersection(self._requiredAttributes + \
                ...
                for key, value in self._unvalidatedRequest.items():
                    ...
                    if len(match) == 1:
                        ...
                        log("[GetPendingTodos VALIDATION] Failed due more than 1 match for Attribute: " + self._validatedRequest[key], "ERROR")
                        raise Exception(errorResponse)
            else:
                log("[GetPendingTodos VALIDATION] Failed due to required Attributes not present #2, required: " + \
                    str(self._requiredAttributes) + ", request: " + str(self._unvalidatedRequest), "ERROR")
                raise Exception(errorResponse)
        log("[GetPendingTodos VALIDATION] Success" + str(self._validatedRequest), "INFO")
    ...
    def get(self):
        try:
            ...
            log("[GetPendingTodos] Successful, todo: " + str(pending_todos), "INFO")
            return {
                "statusCode": 200,
                "body": json.dumps({
                    "todos": pending_todos
            ...
            log("[GetPendingTodos] Get failed for user with email={email} with error: {err}"\
